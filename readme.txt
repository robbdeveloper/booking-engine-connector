...
= 0.3.3 =
* **Unit categories**: Harden synced category deduplication — manual AJAX sync now runs the same category registry preflight as full sync; canonical terms resolve strictly by `bec_provider_slug` + `bec_external_id`; existing category slugs are preserved on update; new terms use collision-resistant slugs; duplicate translated category terms are merged automatically; unit assignment fails closed when a category cannot be resolved.
